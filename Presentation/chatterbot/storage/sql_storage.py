...
class SQLStorageAdapter(StorageAdapter):
    ...
    def get_random(self, conversation=None):
        """
        ...
        count = self.count()
        if conversation:
            statements = session.query(Statement).filter(Statement.conversation == conversation)
            count = statements.count()
        if count < 1:
            raise self.EmptyDatabaseException()
        random_index = random.randrange(0, count)
        random_statement = statements[random_index]
